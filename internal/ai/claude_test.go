...
func TestQueryClaude_success(t *testing.T) {
	...
	got, _, err := QueryClaude(context.Background(), "test-key", "", "analyze this plan")
	if err != nil {
		t.Fatal(err)
	}
...
func TestQueryClaude_httpError(t *testing.T) {
	...
	_, _, err := QueryClaude(context.Background(), "bad-key", "", "prompt")
	if err == nil {
...
func TestQueryClaude_timeout(t *testing.T) {
	...
	_, _, err := QueryClaude(ctx, "key", "", "prompt")
	if err == nil {
